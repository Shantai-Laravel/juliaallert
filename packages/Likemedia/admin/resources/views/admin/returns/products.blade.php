      ...
      @foreach ($orders as $order)
              ...
              @if (!in_array($orderProduct->id, $orders_id))
                  @if ($orderProduct->subproduct)
                        @if ($orderProduct->orderSet)
                          <option data-subproduct_id="{{$orderProduct->subproduct_id}}" data-product_id="{{$orderProduct->product_id}}" value="{{$orderProduct->id}}">{{$orderProduct->product->translationByLanguage($lang->id)->first()->name }} - {{ $orderProduct->qty }} item from {{$orderProduct->orderSet->set->translation($lang->id)->first()->name}}</option>
                        @else
          ...
          @if (count($order->orderSets) > 0)
            @foreach ($order->orderSets as $orderSet)
              @if (!in_array($orderSet->order_id, $orders_id))
