...
class QuickUploadController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $view = view('admin::admin.quickUpload.productsMore', compact('products', 'sets', 'categories', 'category', 'properties'))->render();
            $url = $products->nextPageUrl();
            return response()->json(['html'=>$view, 'url'=>$url, 'lastItem' => $lastItem]);
        }

        return view('admin::admin.quickUpload.index', compact('products', 'categories', 'category', 'brands', 'sets', 'promotions', 'properties'));
    }
    ...
    public function saveProducts(Request $request)
    {
        ...
        if (!is_null($product)) {
            Product::where('id', $id)->update([
                        ...
                        'actual_price_lei' => $price_lei - ($price_lei * $discount / 100),
                        'discount' => $discount,
                        'code' => $code,
                        'stock' => $stock,
                        'video' => $video,
            ...
            SetProducts::where('product_id', $product->id)->delete();
            if (!empty(json_decode($request->get('set')))) {
                foreach (json_decode($request->get('set')) as $key => $set) {
                    SetProducts::create([
                        'set_id' => $set,
                        'product_id' => $product->id,
        ...
        return view('admin::admin.quickUpload.toClone', compact('products', 'categories', 'brands', 'sets', 'promotions', 'properties'))->render();
    }
        ...
        if (!is_null($category)) {
            ...
            if (count($sets)) {
                foreach ($sets as $key => $set) {
                    $setString .= $set->translation(1)->first()->name.'( '.$set->id.' ), ';
                }
            }

            // default values
            $values = [$category->translationByLanguage(1)->first()->name, 'string', 'стриг', 'стриг', 'стриг', 'стриг', 'int (euro)', 'int (lei)', 'int (%)', 'int', 'youtube video id', $setString ];
                ...
                if ($data['subprod']) {
                    foreach ($data['subprod'] as $key => $subprod) {
                        $product->subproducts()->where('id', $key)->update([
                            ...
                            'price' => $product->price,
                            'price_lei' => $product->price_lei,
                            'actual_price' => $product->price - ($product->price * $product->discount / 100),
                            'actual_price_lei' => $product->price_lei - ($product->price_lei * $product->discout / 100),
                            'discount' => $product->discount,
                            'stock' => $product->stock,
                        ]);
                    }
